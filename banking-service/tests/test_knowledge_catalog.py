...
def test_local_guidance_bundle_for_voice_fraud_returns_expected_topics():
    ...
    assert bundle["content_version"] == "2.7"
    ...
    assert "prepare_action_proposal" in bundle["agent_guidance_summary"]
    assert "commit_action_proposal" in bundle["agent_guidance_summary"]
    ...
    assert "triage_fraud_case" not in bundle["agent_guidance_summary"]
...
def test_local_guidance_release_validation_passes_current_bundle():
    ...
    assert result["bundle_version"] == "2.7"
