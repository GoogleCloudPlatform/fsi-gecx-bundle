...
async def answers_endpoint(
    ...
    try:
        ...
        # Construct dynamic user profile prompt and end_user_spec if profile exists
        end_user_spec = None
        user_id = None
        if token and token.user_id:
            ...
            try:
                # TODO: Cache this
                from utils.bq import get_customer_from_bigquery
                customer = get_customer_from_bigquery(token.user_id)
                if customer:
                    profile_details = []
        ...
        # Parse query ID from response answer name (projects/.../engines/.../sessions/.../answers/answer-id)
        query_id = ""
        if response.answer and response.answer.name:
            query_id = response.answer.name.split("/")[-1]

        # Parse related questions from the response
        related_questions = list(
            response.answer.related_questions) if response.answer and response.answer.related_questions else []
