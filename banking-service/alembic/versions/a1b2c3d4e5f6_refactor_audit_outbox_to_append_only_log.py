...
def upgrade() -> None:
    ...
    # 1. Drop status mutation columns and old index on status
    op.drop_index('idx_audit_outbox_status', table_name='audit_outbox', schema='audit')
...
def downgrade() -> None:
    ...
    op.create_index('idx_audit_outbox_status', 'audit_outbox', ['status'], unique=False, schema='audit')
