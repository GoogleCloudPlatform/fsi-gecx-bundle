...
def _clear_current_proposal(callback_context) -> None:
    ...
    callback_context.variables["proposal_presentations_json"] = ""
...
def after_tool_callback(tool, input, callback_context, tool_response):
    ...
    if payload.get("success") is True and proposal_id and summary:
        ...
        callback_context.variables["proposal_presentations_json"] = json.dumps(presentations)
        locale = callback_context.variables.get("runtime_language_code") or "en-US"
        localized = presentations.get(locale, {})
        summary = localized.get("speech_text") or summary
        invocation_id = str(callback_context.invocation_id or "")
