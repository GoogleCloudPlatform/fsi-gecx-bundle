    ...
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center p-6 text-center text-slate-900 dark:text-white">
        <div className="max-w-md space-y-6">
          ...
          <AnalyticsButton analyticsId="accounts_view_back_to_home" 
            onClick={() => navigate('/')}
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 pt-28 pb-12 px-6 md:pt-36 md:pb-20 md:px-12">
      <div className="max-w-6xl mx-auto space-y-12">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 pb-6 border-b border-slate-200 dark:border-slate-900">
          ...
          {selectedAccountId && (
            <AnalyticsButton analyticsId="accounts_view_back_to_accounts" 
        ...
        {isLoading ? (
          ...
          <div className="max-w-md mx-auto bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-850 rounded-3xl p-8 text-center space-y-6 shadow-xl dark:shadow-none">
            ...
            <AnalyticsButton analyticsId="accounts_view_provision_demo_suite" 
              onClick={async () => {
                setIsLoading(true);
                try {
          ...
          <div className="space-y-8">
            ...
            <div className="bg-gradient-to-r from-emerald-500/10 to-teal-500/10 border border-emerald-500/25 rounded-3xl p-8 flex flex-col sm:flex-row items-center justify-between gap-6">
              ...
              <AnalyticsButton analyticsId="accounts_view_boost_yield_now" 
                onClick={() => {
          ...
          <div className="space-y-8">
                ...
                <div className="space-y-6">
                  <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-5">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                      <AnalyticsButton analyticsId="accounts_view_pay_bill" 
                      ...
                      <AnalyticsButton analyticsId="accounts_view_statements" 
              ...
              (activeAccountObj?.cards || []).length > 0 && (
                <div className="grid max-w-3xl gap-4 sm:grid-cols-2">
                  {((activeAccountObj.cards || []).filter(card => card.status === 'ACTIVE').length > 0
                    ...
                    <div
                      key={card.card_id || card.id || card.card_token || card.last_four}
                      className="rounded-2xl border border-slate-200 bg-white p-4 text-slate-900 shadow-sm dark:border-slate-800 dark:bg-slate-900/50 dark:text-white"
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <span className="shrink-0 rounded-full bg-emerald-100 px-2 py-1 text-[10px] font-bold uppercase text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300">
                          {card.status || 'Active'}
                        </span>
                      </div>
                      <div className="mt-3 flex flex-wrap gap-2">
                        <span className="rounded-full bg-indigo-100 px-2.5 py-1 text-[10px] font-bold uppercase text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300">
                          {card.is_virtual ? 'Virtual' : 'Physical'}
                        </span>
                        {!card.is_virtual && (
                          <span className="rounded-full bg-slate-200 px-2.5 py-1 text-[10px] font-bold uppercase text-slate-700 dark:bg-slate-800 dark:text-slate-300">
                            Primary card
                          </span>
                        )}
                      </div>
            ...
            <div className="bg-white dark:bg-slate-900/40 border border-slate-205 dark:border-slate-850 rounded-3xl p-6 md:p-8 shadow-sm dark:shadow-none">
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-5">
                ...
                {selectedAccountType === 'credit' && (
                  <AnalyticsButton analyticsId="accounts_view_spend_analyzer"
              ...
              {selectedAccountType === 'credit' && (
                <div className="mb-5 flex gap-2 overflow-x-auto pb-1">
                  {ledgerQuickTabs.map(tab => (
                    <AnalyticsButton analyticsId="accounts_view_08"
                      key={tab.id}
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 mb-6">
                <div className="relative flex-1">
                  ...
                  {searchQuery && (
                    <AnalyticsButton analyticsId="accounts_view_clear"
                ...
                <div className="flex items-center gap-2">
                  <AnalyticsButton analyticsId="accounts_view_10"
              ...
              {isFilterMenuOpen && (
                <div className="mb-8 p-6 bg-slate-50/95 dark:bg-slate-900/95 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-xl animate-fadeIn space-y-6">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 dark:border-slate-800 pb-4">
                    <div className="flex items-center gap-1.5 overflow-x-auto pb-1 sm:pb-0">
                      {[
                        ...
                        <AnalyticsButton analyticsId="accounts_view_11"
                          key={tab.id}
                    ...
                    {activeFilterCount > 0 && (
                      <AnalyticsButton analyticsId="accounts_view_reset_all_filters_active_filter_count"
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {[
                            ...
                            <AnalyticsButton analyticsId="accounts_view_13"
                              key={cat.id}
                    ...
                    {activeFilterTab === 'amount' && (
                      <div className="space-y-4 max-w-lg">
                        ...
                        <div className="flex flex-wrap gap-2 pt-1">
                          <AnalyticsButton analyticsId="accounts_view_under_25"
                          ...
                          <AnalyticsButton analyticsId="accounts_view_25_100"
                            onClick={() => setFilters(prev => ({ ...prev, minAmount: '25', maxAmount: '100' }))}
                            className="px-3.5 py-1.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer"
                          >
                            $25 - $100
                          </AnalyticsButton>
                          <AnalyticsButton analyticsId="accounts_view_over_100"
                            onClick={() => setFilters(prev => ({ ...prev, minAmount: '100', maxAmount: '' }))}
                            className="px-3.5 py-1.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer"
                          >
                            Over $100
                          </AnalyticsButton>
                          <AnalyticsButton analyticsId="accounts_view_clear_amount"
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {[
                            ...
                            <AnalyticsButton analyticsId="accounts_view_18"
                              key={d.id}
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {[
                            ...
                            <AnalyticsButton analyticsId="accounts_view_19"
                              key={c.id}
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {[
                            ...
                            <AnalyticsButton analyticsId="accounts_view_20"
                              key={s.id}
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-slate-200 dark:border-slate-800 text-xs">
                    ...
                    <AnalyticsButton analyticsId="accounts_view_apply_close_panel"
                                    ...
                                    <React.Fragment key={rowKey}>
                                      <tr className="hover:bg-slate-100/50 dark:hover:bg-slate-900/20 transition-colors">
                                        ...
                                        <td className="py-3 font-medium text-slate-800 dark:text-slate-300 w-[44%]">
                                          <AnalyticsButton analyticsId="accounts_view_22"
                                          ...
                                          <td colSpan={5} className="pb-4">
                                            <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/40 p-4 text-xs text-slate-600 dark:text-slate-300">
                                              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                                                ...
                                                <AnalyticsButton analyticsId="accounts_view_report_or_dispute" className="text-left font-bold text-blue-650 dark:text-blue-300 hover:underline cursor-pointer">Report or dispute</AnalyticsButton>
                                              </div>
                                            </div>
                                          </td>
                                        </tr>
                                  ...
                                  <React.Fragment key={rowKey}>
                                    <tr className="hover:bg-slate-100/50 dark:hover:bg-slate-900/30 transition-colors">
                                      ...
                                      <td className="py-4 font-medium text-slate-800 dark:text-slate-200 w-[44%]">
                                        <AnalyticsButton analyticsId="accounts_view_24"
                                          onClick={() => setExpandedTransactionKey(isExpanded ? null : rowKey)}
                                          className="flex w-full min-w-0 items-center gap-2 text-left cursor-pointer"
                                        >
                                          {isExpanded ? <ChevronDown className="w-4 h-4 flex-shrink-0 text-slate-400" /> : <ChevronRight className="w-4 h-4 flex-shrink-0 text-slate-400" />}
            ...
            {selectedAccountType === 'credit' && (
              <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <AnalyticsButton analyticsId="accounts_view_learn_more"
                  onClick={() => {
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-6 z-50 animate-fade-in">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-lg w-full p-8 space-y-6 shadow-2xl">
            <div className="flex justify-between items-start">
              ...
              <AnalyticsButton analyticsId="accounts_view_27" 
            ...
            <AnalyticsButton analyticsId="accounts_view_close_details" 
              onClick={() => setShowDocModal(false)}
